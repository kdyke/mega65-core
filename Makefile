...
MONITORVERILOG=		$(VERILOGSRCDIR)/6502/6502_alu.v \
			$(VERILOGSRCDIR)/6502/6502_mux.v \
			$(VERILOGSRCDIR)/6502/6502_reg.v \
			$(VERILOGSRCDIR)/6502/6502_timing.v \
			$(VERILOGSRCDIR)/6502/6502_top.v \
			$(VERILOGSRCDIR)/6502/6502_ucode.v \
			$(VERILOGSRCDIR)/monitor/monitor_top.v \
...
$(VHDLSRCDIR)/shadowram.vhdl:	$(TOOLDIR)/mempacker/mempacker_new $(SDCARD_DIR)/BANNER.M65 $(ASSETS)/alphatest.bin Makefile
	mkdir -p $(SDCARD_DIR)
	$(TOOLDIR)/mempacker/mempacker_new -n shadowram -s 131071 -f $(VHDLSRCDIR)/shadowram.vhdl $(SDCARD_DIR)/BANNER.M65@3D00 $(ASSETS)/alphatest.bin@0400
...
vivado/%.xpr: 	vivado/%_gen.tcl | $(VHDLSRCDIR)/*.vhdl $(VHDLSRCDIR)/*.xdc $(SIMULATIONVHDL) $(MONITORVERILOG)
	echo MOOSE $@ from $<
	$(VIVADO) -mode batch -source $<

$(BINDIR)/%.bit: 	vivado/%.xpr $(VHDLSRCDIR)/*.vhdl $(VHDLSRCDIR)/*.xdc $(SIMULATIONVHDL) $(MONITORVERILOG)
	echo MOOSE $@ from $<
#	@rm -f $@
#	@echo "---------------------------------------------------------"
